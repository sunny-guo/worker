...
//		ThreadLocalMetricsRecorder.getInstance().setStatus(statusLine.getVersion()+","+statusLine.getStatusCode());
		ThreadLocalMetricsRecorder.getInstance().setStatus(statusLine.getStatusCode()+"");
...
//		HTTP.LOGGER.debug("Response:\r\n{}", buf.toString());
		ThreadLocalMetricsRecorder.getInstance().setRes(buf.toString());
//		System.out.println("response长度"+buf.toString().getBytes().length);
	}
