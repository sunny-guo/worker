...
public class StringUtil {
     ...
     * 替换实现.
     * 
     * @see replace(String oldStr,String findStr,String
     *      replStr,int times)
     * @see replace(String oldStr,String findStr,String
     *      replStr)
     */
     ...
     * @param beginStr 开始字串.
     * @param endStr 终止字串.
     * @param times 删除指定次数,如小于1,立即返回;否则一直匹配到指定次数为止
     * @return 删除后的字串.
     */
     ...
     * 获得不含路径的文件名.
     * 
     * @param fileName 文件名,可能含，也可能不含路径.为空/空串返回空串
     * @return 不含路径的文件名.
     */
    ...
    /**
     * 进行二维数组某一列字符串的替换. 使用示例： String[][] string= new
     * String[][]{{"string", "cying"},{"ss-strstd",
     * "string"}, {"dir-str", ""},{"teacher",""}};
     * String[][] newStr = ReplaceString.replace(str, 0,
     * "str", "cry"); 你将会得到这样的结果数组： newStr = {{"crying",
     * "cying"},{"ss-crystd", "string"}, {"dir-cry",
     * ""},{"teacher",""}};
     * 
     * @param str 要进行替换的字符串
     * @param columnIndex 要进行替换的字符串所在的列数 columnIndex从0开始
     ...
     * </code> </p>
     * 
     * @param packageName 包名,packageName为null或一空串时，返回null
     ...
     * 需要注意的是，如果任何下面的情况出现了，会抛出空指针异常（NullPointerException），
     * 但源数组与目标都保持不便： 如果dst为null； 如果src为null.
     * 如果任何下面的情况出现了，会抛出数组越界异常（IndexOutOfBoundsException），而
     * 源数组与目标数组保持不变： 如果srcOffset为负数； 如果dstOffset为负数；
     * 如果length为负数； 如果srcOffset + length大于src.length，
     * 如果dstOffset + length大于dst.length.
     * 
     * @param src 源数组
     * @param src_position 源数组起始位置
     * @param dst 目标数组
     * @param dst_position 目标数组起始位置
     * @param length 长度
     * @throws NullPointerException (如上)
     * @throws IndexOutOfBoundsException (如上)
     */
     ...
     * 但源数组保持不便： 如果src为null.
     * 如果任何下面的情况出现了，会抛出数组越界异常（IndexOutOfBoundsException），而
     * 源数组保持不变： 如果srcOffset为负数； 如果length为负数； 如果srcOffset +
     * length大于src.length，
     * 
     * @param src 源数组
     * @param src_position 源数组起始位置
     ...
     * 
     * @param list 要合并的数组的列表,要求数组列数必须相等.
     * @return 合并后的数组.
     */
   ...
   /**
     ...
     * 
     * @param arr 要连接的数组,如长度为1,返回该元素,否则，返回用分隔符隔开而成的字串.
     * @param delim 分隔符.
     * @return 将数组转换成字串，以便可执行逆操作split().
     * @exception AppException 当分隔符为null或空串时抛出.
     */
    public static String join(String[] arr, String delim) {
        ...
        if (delim.length() < 1) {
            try {
				throw new Exception("参数非法，连接数组时，分隔符不可为null或空串");
			} catch (Exception e) {
    ...
    public final static String extractNumber(String in) {
        ...
        return (result.length() == 0) ? "0" : result.toString(); // if
                                                                 // nothing
                                                                 // left,
                                                                 // return
                                                                 // 0
     ...
     *         HashMap<String,String>(); map.put("id",
     *         "111111111"); map.put("data", "ssssss");
     *         StringUtil.applyTemplate(
